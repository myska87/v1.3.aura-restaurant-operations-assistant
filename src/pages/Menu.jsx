...
import MenuItemEditDialog from "@/components/MenuItemEditDialog"; // Assuming this path for the dialog component
...
export default function Menu() {
  ...
  const filteredItems = menuItems.filter(item => {
    ...
    // Only show active items unless in manager view (e.g., in a management page)
    // For this public facing menu, we only show active items.
  ...
  const handleQuickToggle = async (item, e) => {
    e.stopPropagation();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6 md:p-8">
      ...
      {isLoading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item) => (
            <Card
              key={item.id}
              className="bg-white border-none shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group cursor-pointer relative"
              onClick={() => navigate(createPageUrl(`MenuItemView?id=${item.id}`))}
            >
              {/* Floating Edit Button - Manager Only */}
              {isManager && (
                <div className="absolute top-2 left-2 z-10 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <Button
                    ...
                    size="icon"
                    className="bg-white/95 hover:bg-white shadow-lg"
                    onClick={(e) => handleQuickToggle(item, e)}
                  >
              ...
              <div className="relative h-48 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">
                ...
                <div className="absolute top-2 right-2 flex gap-1 flex-wrap justify-end">
                  ...
                  {item.is_active === false && (
                    <Badge className="bg-gray-100 text-gray-600 border-gray-300 shadow-sm">
                      Inactive
                    </Badge>
                  )}
                </div>
      ...
      <MenuItemEditDialog
        ...
        onClose={() => {
          ...
          setEditingItem(null);
          queryClient.invalidateQueries({ queryKey: ['menuItems'] }); // Invalidate to refresh the list after potential edit
        }}
      />
    </div>
  );
